namespace Psequel {
    public class SQLCompletionProvider : Object, GtkSource.CompletionProvider {
        ...
        private ObservableList<Model> candidates;
        ...
        public SQLCompletionProvider () {
            ...
            candidates = new ObservableList<Model> ();
            for (int i = 0; i < PGListerals.KEYWORDS.length; i++) {
                candidates.append (new Model (PGListerals.KEYWORDS[i], "KEYWORD"));
            }

            for (int i = 0; i < PGListerals.FUNCTIONS.length; i++) {
                candidates.append (new Model (PGListerals.FUNCTIONS[i], "FUNCTION"));
            }

            for (int i = 0; i < PGListerals.DATATYPES.length; i++) {
                candidates.append (new Model (PGListerals.DATATYPES[i], "DATATYPE"));
            }

            for (int i = 0; i < PGListerals.RESERVED.length; i++) {
                candidates.append (new Model (PGListerals.RESERVED[i], "RESERVED"));
            }
            ...
            model = new Gtk.FilterListModel (candidates, filter);
        }
